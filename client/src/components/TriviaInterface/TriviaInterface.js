...
const TriviaInterface = ({ isHost, isTeamLeader }) => {
  const {
    ...
    hasSubmittedTrivia,
    setHasSubmittedTrivia,
    submittedTriviaAnswers,
    myTeamId,
    disableGetTriviaQuestion,
  } = useContext(Context);
          ...
          {triviaQuestion && (
            <Box sx={boxStyling}>
              <Typography
                align="center"
                variant="h6"
              >{`Trivia Question: ${triviaQuestion.Question}`}</Typography>
              <br />
              <Typography
                align="center"
                variant="h6"
              >{`Option 1: ${triviaQuestion.Option1}`}</Typography>
              <Typography
                align="center"
                variant="h6"
              >{`Option 2: ${triviaQuestion.Option2}`}</Typography>

              {triviaQuestion.Option3 && (
                <Typography
                  align="center"
                  variant="h6"
                >{`Option 3: ${triviaQuestion.Option3}`}</Typography>
              )}
              {triviaQuestion.Option4 && (
                <Typography
                  align="center"
                  variant="h6"
                >{`Option 4: ${triviaQuestion.Option4}`}</Typography>
              )}
              <br />
        ...
        <Box align="center">
          <ButtonGroup variant="text" aria-label="text button group">
            ...
            <Button
              ...
              disabled={disableGetTriviaQuestion}
            >
        ...
        <Box sx={boxStyling}>
          {triviaQuestion ? (
            <Box sx={boxStyling}>
              ...
              <FormControl fullWidth>
                ...
                >
                  ...
                  {triviaQuestion.Option3 && (
                    <MenuItem value={triviaQuestion.Option3}>{triviaQuestion.Option3}</MenuItem>
                  )}
                  {triviaQuestion.Option3 && (
                    <MenuItem value={triviaQuestion.Option4}>{triviaQuestion.Option4}</MenuItem>
                  )}
                </Select>
              </FormControl>
              <br /> <br />
              {isTeamLeader ? (
                ...
                <Typography>
                  You are not the current team leader, discuss with your team to help your team
                  leader pick the right answer.
                </Typography>
              )}
            ...
            <Typography
              align="center"
              variant="h6"
            >{`Waiting for a trivia question...`}</Typography>
          )}
        </Box>
      </Box>
    );
